...
async function crearProfesional(req, res, service) {
  try {
    const { nombre, apellido, dni, matricula, especialidad, id} = req.body;
    if (!nombre || !apellido || !dni) {
      return res.status(400).json({ error: 'Faltan campos requeridos' });
    }

    const profesional = new Profesional(nombre, apellido, dni, matricula, especialidad, id);
    await service.crear(profesional);
    logger.info('Profesional creado con ID: %d', profesional.id);
...
async function obtenerProfesional(req, res, service) {
  try {
    ...
    logger.info('Profesional obtenido con ID: %d', profesional.id);
